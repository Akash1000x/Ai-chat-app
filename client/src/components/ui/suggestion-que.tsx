...
import { useGetSuggestionQuestions } from "@/hooks/api/get-suggestioin-questions"
...
export default function SuggestionQueue({
  ...
  return (
    <div className={`w-full max-w-3xl px-4 m-auto h-full flex flex-col justify-center space-y-2 transition-all duration-500 ease-in-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
      }`}>
      <h1 className="text-3xl md:text-4xl font-semibold text-foreground">
        How can I help you{username && <span className="text-primary capitalize">{`, ${username}`}</span>}?
      </h1>

...
export default function Chat({ conversationId }: { conversationId?: string }) {
  ...
  const [messages, setMessages] = React.useState<MessageType[]>(queryData || [])
  ...
  useEffect(() => {
    if (isAtBottom && aiResponse) {
      scrollToBottom("instant")
    }
  }, [aiResponse, isAtBottom, scrollToBottom])
  ...
  const handleSubmit = useCallback(
    async (promptData: { message: string; model: Model }) => {
      if (!session?.session) {
        navigate({
          to: "/auth/sign-in",
        })
      ...
      if (!conversationId) {
        const { id }: { id: string; title: string } =
    ...
    [
      ...
      queryClient,
      messages,
      API_BASE_URL, // Added this dependency
    ],
  )
...
const MessageItem = React.memo(
  ({
    ...
    message: MessageType
    models: ModelsCategory[] | undefined
  }) => {
